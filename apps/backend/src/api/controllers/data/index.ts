        ...
        async (req) => {
            ...
            try {
                ...
                const moveItem = async (item: Data, targetFolderId: string, session: mongoose.ClientSession) => {
                    if (isFolder(item)) {
                        await FolderModel.updateOne({ _id: item.id }, { parentFolderId: targetFolderId }, { session });
                    } else if (isFileMeta(item)) {
                        await FileModel.updateOne({ _id: item.id }, { parentFolderId: targetFolderId }, { session });
                    }
                };
